...
void extendBlossom(edge **graph, blossom *currBlossom, int *matching, int *trueVertex) { // reversing blossom contraction
    ...
    // true vertices now pointing on their real vertex (or contracted if there were contraction in contraction)
    cycleNode *currNode = currBlossom -> baseNode -> next;
...
int *findMatchingGreedy(edge **graph, int citizensNum) { // to start faster
...
int *blossomAlgorithm(edge **graph, int SIZE) {
    int *matching = findMatchingGreedy(graph, SIZE);
        ...
        for (int i=0; i<SIZE; i++){
            ...
            if (vertices[i] == nullptr && i == trueVertex[i]) {
                ...
                while(currNeigh != nullptr) {
                    ...
                    if (!visited[trueVertex[currNeigh->vertex]] && vertices[trueVertex[currNeigh->vertex]] != nullptr &&
                        ...
                        visited[trueVertex[currNeigh->vertex]] = true;
            ...
            for (int i=0; i<SIZE; i++) {
                ...
                if (vertices[i] != nullptr && i == trueVertex[i] && vertices[i] -> loc == outer) {
                    ...
                    while(currNeigh != nullptr) {
                        ...
                        if (!visited[trueVertex[currNeigh->vertex]] &&
                            vertices[trueVertex[currNeigh->vertex]] != nullptr && i != trueVertex[currNeigh->vertex]
                            && vertices[trueVertex[currNeigh->vertex]] -> loc == outer) {
